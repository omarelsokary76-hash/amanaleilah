...
// ================= تشفير الـ PIN وكود الاسترجاع (PBKDF2) =================
// بنخزن الـ PIN وكود الاسترجاع كـ hash بس، مش نص صريح، حتى إحنا كمطورين ما نقدرش نشوفهم.
...
// ================= حماية من محاولات تخمين كود العائلة / رقم الهاتف (Brute-force) =================
// بيستخدم Cloudflare Rate Limiting binding (مُعرّف في wrangler.toml تحت [[ratelimits]]).
// بيحد كل IP بعدد محاولات محدودة على شاشات الدخول/التسجيل/إنشاء العائلة في نفس الدقيقة.
async function checkAuthRateLimit(request, env) {
  try {
    if (!env.AUTH_LIMITER) return true; // لو الـ binding مش متسجل لسه، منسمحش بفشل التطبيق كله (fail-open مؤقت)
    const ip = request.headers.get('CF-Connecting-IP') || 'unknown-ip';
    const { success } = await env.AUTH_LIMITER.limit({ key: ip });
    return success;
  } catch (e) {
    return true; // أي عطل في خدمة الـ rate limiting نفسها ميوقفش تسجيل الدخول العادي
  }
}
...
// لو المفتاح مش متسجل، مفيش تشفير هيحصل (fail open) عشان التطبيق يفضل شغال، لكن ده مؤقت لحد
// ما تسجل المفتاح - لازم تسجله قبل ما تنزل التطبيق فعليًا للناس.
...
async function encryptGeo(env, value) {
  ...
  if (!key) return String(value); // مفيش مفتاح متسجل لسه - بيتخزن زي ما هو مؤقتًا
...
const VAPID_PRIVATE_KEY = 'rtzzawlOWnXkseiMvx_r5SqNdNjRy9aF2QxPyLrGTt8';
...
async function importVapidPrivateKey() {
  ...
  const d = b64urlToUint8(VAPID_PRIVATE_KEY);
  const jwk = {
...
async function notifyFamilySOS(env, familyCode, excludeMemberId, memberName) {
  try {
    ...
    await Promise.all(results.map(row => sendWebPush({
...
const CACHE_NAME = 'amanaleilah-shell-v2';
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path.startsWith('/api/admin/')) {
        ...
        // دور على عائلة برقم هاتف عضو فيها (مفيد لو حد نسي كود العائلة بتاعه)
        if (path === '/api/admin/lookup-phone' && method === 'GET') {
          const phone = (url.searchParams.get('phone') || '').trim();
          if (!phone) return json({ error: 'اكتب رقم الهاتف' }, 400);
          const { results } = await env.DB.prepare(
            'SELECT family_code, name, relation, is_admin FROM members WHERE phone = ?'
          ).bind(phone).all();
          return json({ matches: results.map(r => ({
            familyCode: r.family_code, name: r.name, relation: r.relation, isAdmin: !!r.is_admin,
          })) });
        }
      ...
      if (path === '/api/family/create' && method === 'POST') {
        ...
        const { name, phone, relation, consent, pin } = body;
        ...
        if (!pin || !/^[0-9]{4,6}$/.test(pin)) return json({ error: 'الرقم السري (PIN) لازم يكون من 4 لـ 6 أرقام' }, 400);
        ...
        await env.DB.prepare(
          `INSERT INTO members (id, family_code, name, phone, relation, role, is_admin, status, circle, pin_hash, recovery_code_hash, created_at)
           VALUES (?, ?, ?, ?, ?, 'parent', 1, 'approved', '[]', ?, ?, ?)`
        ).bind(id, code, name, phone, relation, pinHash, recoveryHash, now).run();

        const family = await getFamilyWithMembers(env.DB, code);
        // كود الاسترجاع بيظهر مرة واحدة بس هنا - السيرفر مش هيرجعه تاني أبدًا بعد كده
        return json({ family, myId: id, recoveryCode });
      }
      ...
      if (membersLiteMatch && method === 'GET') {
        const { results } = await env.DB.prepare(
          'SELECT id, name, relation, is_admin, pin_hash FROM members WHERE family_code = ?'
        ).bind(membersLiteMatch[1]).all();
        if (!results.length) return json({ error: 'كود العائلة غير صحيح' }, 404);
        return json({ members: results.map(m => ({
          id: m.id, name: m.name, relation: m.relation, isAdmin: !!m.is_admin, hasPin: !!m.pin_hash,
        })) });
      }

      // ---------- تسجيل دخول (عضو موجود) - بالـ PIN بدل رقم الهاتف ----------
      if (path === '/api/family/login' && method === 'POST') {
        if (!(await checkAuthRateLimit(request, env))) {
          return json({ error: 'محاولات كتير أوي، حاول تاني بعد شوية' }, 429);
        }
        const { code, memberId, pin } = await request.json();
        const upperCode = (code || '').toUpperCase();
        const memberRow = await env.DB.prepare(
          'SELECT * FROM members WHERE id = ? AND family_code = ?'
        ).bind(memberId, upperCode).first();
        if (!memberRow) return json({ error: 'كود العائلة أو العضو غير صحيح' }, 404);
        if (!memberRow.pin_hash) return json({ error: 'no_pin_set' }, 409); // لسه محددش PIN (عضو جديد أو اتعمله reset)
        const valid = await verifySecret(pin, memberRow.pin_hash);
        if (!valid) return json({ error: 'الرقم السري غلط' }, 401);
        const family = await getFamilyWithMembers(env.DB, upperCode);
        return json({ family, myId: memberRow.id });
      }

      // ---------- تحديد PIN لأول مرة (عضو جديد لسه محددوش، أو بعد ما كبير العائلة يعمله reset) ----------
      if (path === '/api/family/set-initial-pin' && method === 'POST') {
        if (!(await checkAuthRateLimit(request, env))) {
          return json({ error: 'محاولات كتير أوي، حاول تاني بعد شوية' }, 429);
        }
        const { code, memberId, pin } = await request.json();
        ...
        const pinHash = await hashSecret(pin);
        await env.DB.prepare('UPDATE members SET pin_hash = ? WHERE id = ?').bind(pinHash, memberId).run();
        const family = await getFamilyWithMembers(env.DB, upperCode);
        return json({ family, myId: memberId });
      }

      // ---------- تغيير الـ PIN وانت داخل حسابك بالفعل (من غير ما تحتاج تكتب القديم) ----------
      const changePinMatch = path.match(/^\/api\/member\/([A-Z0-9]+)\/([a-zA-Z0-9_]+)\/change-pin$/);
      if (changePinMatch && method === 'POST') {
        const { newPin } = await request.json();
        if (!newPin || !/^[0-9]{4,6}$/.test(newPin)) return json({ error: 'الرقم السري لازم يكون من 4 لـ 6 أرقام' }, 400);
        const memberRow = await env.DB.prepare(
          'SELECT id FROM members WHERE id = ? AND family_code = ?'
        ).bind(changePinMatch[2], changePinMatch[1]).first();
        if (!memberRow) return json({ error: 'العضو مش موجود' }, 404);
        await env.DB.prepare('UPDATE members SET pin_hash = ? WHERE id = ?').bind(await hashSecret(newPin), memberRow.id).run();
        return json({ ok: true });
      }

      // ---------- كبير العائلة يصفّر الـ PIN بتاع عضو نسيه (العضو بعدها هيحدد PIN جديد أول ما يحاول يدخل) ----------
      ...
      // ---------- كبير العائلة نسي الـ PIN بتاعه - استرجاع بكود الاسترجاع اللي اتديله وقت إنشاء الحساب ----------
      if (path === '/api/family/admin-recovery' && method === 'POST') {
        if (!(await checkAuthRateLimit(request, env))) {
          return json({ error: 'محاولات كتير أوي، حاول تاني بعد شوية' }, 429);
        }
        const { code, recoveryCode, newPin } = await request.json();
        const upperCode = (code || '').toUpperCase();
        if (!newPin || !/^[0-9]{4,6}$/.test(newPin)) return json({ error: 'الرقم السري الجديد لازم يكون من 4 لـ 6 أرقام' }, 400);
        const memberRow = await env.DB.prepare(
          'SELECT * FROM members WHERE family_code = ? AND is_admin = 1'
        ).bind(upperCode).first();
        if (!memberRow || !memberRow.recovery_code_hash) return json({ error: 'لا يوجد كود استرجاع لهذه العائلة' }, 404);
        const valid = await verifySecret((recoveryCode || '').toUpperCase(), memberRow.recovery_code_hash);
        if (!valid) return json({ error: 'كود الاسترجاع أو كود العائلة غلط' }, 401);
        // بعد الاسترجاع، بنولّد كود استرجاع جديد وبنرجعه مرة واحدة، عشان الكود القديم يبقى مستخدم مرة واحدة بس
        const newRecoveryCode = genRecoveryCode();
        await env.DB.prepare(
          'UPDATE members SET pin_hash = ?, recovery_code_hash = ? WHERE id = ?'
        ).bind(await hashSecret(newPin), await hashSecret(newRecoveryCode), memberRow.id).run();
        return json({ ok: true, recoveryCode: newRecoveryCode });
      }
      ...
      if (path === '/api/family/register' && method === 'POST') {
        ...
        const { code, name, phone, relation, consent, pin } = await request.json();
        if (consent !== true) return json({ error: 'يجب الموافقة على شروط استخدام البيانات أولاً' }, 400);
        if (!pin || !/^[0-9]{4,6}$/.test(pin)) return json({ error: 'الرقم السري (PIN) لازم يكون من 4 لـ 6 أرقام' }, 400);
        const upperCode = (code || '').toUpperCase();
        ...
        const now = Date.now();
        await env.DB.prepare(
          `INSERT INTO members (id, family_code, name, phone, relation, role, is_admin, status, circle, pin_hash, created_at)
           VALUES (?, ?, ?, ?, ?, ?, 0, 'approved', '[]', ?, ?)`
        ).bind(id, upperCode, name, phone, relation, role, await hashSecret(pin), now).run();
      ...
      if (eventsMatch && method === 'POST') {
        const body = await request.json();
        const id = genId('e');
        const encLat = await encryptGeo(env, body.lat ?? null);
        const encLng = await encryptGeo(env, body.lng ?? null);
        await env.DB.prepare(
          ...
          body.text || null, encLat, encLng, body.accuracy ?? null, Date.now()
        ).run();
        if (body.type === 'sos') {
  ...
  // ---------- تشغيل تلقائي يومي (Cron Trigger) لحذف البيانات القديمة ----------
