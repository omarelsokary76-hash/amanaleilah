  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/api/push/subscribe' && method === 'POST') {
        ...
        if (!(await requireSession(request, env, fc, memberId))) {
          return json({ error: 'الجلسة منتهية أو غير صالحة، سجّل دخول تاني' }, 401);
        }
        await env.DB.prepare(
      ...
      if (chatMatch && method === 'POST') {
        const body = await request.json();
        if (!(await requireSession(request, env, chatMatch[1], body.senderId))) {
          return json({ error: 'الجلسة منتهية أو غير صالحة، سجّل دخول تاني' }, 401);
        }
        const id = genId('c');
        await env.DB.prepare(
      ...
      if (publicKeyMatch && method === 'POST') {
        ...
        if (!(await requireSession(request, env, familyCode, memberId))) {
          return json({ error: 'الجلسة منتهية أو غير صالحة، سجّل دخول تاني' }, 401);
        }
        const member = await env.DB.prepare(
          'SELECT id FROM members WHERE id = ? AND family_code = ?'
      ...
      if (privateSendMatch && method === 'POST') {
        ...
        if (!(await requireSession(request, env, familyCode, body.senderId))) {
          return json({ error: 'الجلسة منتهية أو غير صالحة، سجّل دخول تاني' }, 401);
        }
      ...
      if (locStartMatch && method === 'POST') {
        ...
        if (!(await requireSession(request, env, locStartMatch[1], body.memberId))) {
          return json({ error: 'الجلسة منتهية أو غير صالحة، سجّل دخول تاني' }, 401);
        }
        const now = Date.now();
      ...
      if (locPingMatch && method === 'POST') {
        const body = await request.json().catch(() => ({}));
        if (!body.memberId) return json({ error: 'بيانات ناقصة' }, 400);
        if (!(await requireSession(request, env, locPingMatch[1], body.memberId))) {
          return json({ error: 'الجلسة منتهية أو غير صالحة، سجّل دخول تاني' }, 401);
        }
      ...
      if (locStopMatch && method === 'POST') {
        const body = await request.json().catch(() => ({}));
        if (!(await requireSession(request, env, locStopMatch[1], body.memberId))) {
          return json({ error: 'الجلسة منتهية أو غير صالحة، سجّل دخول تاني' }, 401);
        }
      ...
      if (path === '/api/private/request' && method === 'POST') {
        ...
        if (!(await requireSession(request, env, familyCode, requesterId))) {
          return json({ error: 'الجلسة منتهية أو غير صالحة، سجّل دخول تاني' }, 401);
        }
      ...
      if (path === '/api/private/approve' && method === 'POST') {
        ...
        if (!(await requireSession(request, env, familyCode, adminId))) {
          return json({ error: 'الجلسة منتهية أو غير صالحة، سجّل دخول تاني' }, 401);
        }
      ...
      if (mediaUploadMatch && method === 'POST') {
        ...
        if (!(await requireSession(request, env, familyCodeVal, body.senderId))) {
          return json({ error: 'الجلسة منتهية أو غير صالحة، سجّل دخول تاني' }, 401);
        }
