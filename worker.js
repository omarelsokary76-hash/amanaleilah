...
async function performFamilyDeletion(env, ctx, familyCode, deletedById, deletedByName, reason) {
  ...
  await env.DB.prepare('DELETE FROM private_chat_reports WHERE family_code = ?').bind(familyCode).run().catch(() => {});
...
async function performMemberDeletion(env, familyCode, member, reason) {
  ...
  await env.DB.prepare('DELETE FROM private_chat_reports WHERE family_code = ? AND (reporter_id = ? OR reported_member_id = ?)').bind(familyCode, member.id, member.id).run().catch(() => {});
...
async function cleanupOldData(env) {
  ...
  await env.DB.prepare('DELETE FROM private_chat_reports WHERE created_at < ?').bind(cutoff).run().catch(() => {});
...
<body>
  ...
  <ul>
    ...
    <li><strong>الشات الخاص</strong> مشفّر من طرف لطرف - حتى فريقنا وكبير العائلة لا يقدروا يقرأوه، إلا لو بلّغت أنت بنفسك عن رسالة معيّنة (بيتكشف نصها بس، بموافقتك، لكبير العائلة للمراجعة)</li>
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/api/family/login' && method === 'POST') {
        ...
        return json({ family, myId: matched.id });
      }
      ...
      // ---------- حذف رسالة خاصة (المرسل بس، أو كبير العائلة كإجراء بعد بلاغ) ----------
      const privateDeleteMatch = path.match(/^\/api\/private\/([A-Z0-9]+)\/([a-zA-Z0-9_]+)$/);
      if (privateDeleteMatch && method === 'DELETE') {
        const body = await request.json().catch(() => ({}));
        const familyCode = privateDeleteMatch[1];
        const messageId = privateDeleteMatch[2];
        const msg = await env.DB.prepare('SELECT * FROM private_messages WHERE id = ? AND family_code = ?').bind(messageId, familyCode).first();
        if (!msg) return json({ error: 'الرسالة مش موجودة' }, 404);
        if (msg.sender_id !== body.requesterId) {
          if (!(await isAdminMember(env.DB, familyCode, body.requesterId))) {
            return json({ error: 'مينفعش تحذف رسالة مش بتاعتك' }, 403);
          }
        }
        await env.DB.prepare('DELETE FROM private_messages WHERE id = ?').bind(messageId).run();
        await env.DB.prepare('DELETE FROM private_chat_reports WHERE message_id = ?').bind(messageId).run().catch(() => {});
        return json({ ok: true });
      }

      // ---------- الإبلاغ عن رسالة في الشات الخاص ----------
      // مهم جدًا من ناحية الخصوصية: السيرفر أصلاً مش قادر يفك تشفير الرسالة (E2E)، فالمُبلِّغ
      // (اللي عنده النص مفكوك على جهازه) هو اللي بيبعت نص الرسالة المُبلَّغ عنها بس مع البلاغ -
      // ده اختياره الشخصي إنه يكشفها لكبير العائلة عشان يراجعها، وباقي المحادثة تفضل مشفرة تمامًا.
      if (path === '/api/private/report' && method === 'POST') {
        const body = await request.json().catch(() => ({}));
        const { familyCode, messageId, reporterId, reportedMemberId, messageText, reason } = body;
        if (!familyCode || !reporterId || !reportedMemberId || !messageText) {
          return json({ error: 'بيانات ناقصة' }, 400);
        }
        await env.DB.prepare(
          `INSERT INTO private_chat_reports (id, family_code, message_id, reporter_id, reported_member_id, message_text, reason, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
        ).bind(genId('pcr'), familyCode, messageId || null, reporterId, reportedMemberId, messageText, reason || null, Date.now()).run();
        ctx.waitUntil(notifyFamilyReport(env, familyCode, 'عضو (شات خاص)'));
        return json({ ok: true });
      }
      // كبير العائلة يراجع بلاغات الشات الخاص (بيشوف بس الرسالة اللي اتبلّغ عنها، مش باقي المحادثة)
      const privateReportsMatch = path.match(/^\/api\/private\/reports\/([A-Z0-9]+)$/);
      if (privateReportsMatch && method === 'GET') {
        const adminId = url.searchParams.get('adminId');
        if (!(await isAdminMember(env.DB, privateReportsMatch[1], adminId))) {
          return json({ error: 'مسموح لكبير العائلة بس' }, 403);
        }
        const { results } = await env.DB.prepare(
          `SELECT r.*, rep.name as reporter_name, tgt.name as reported_name
           FROM private_chat_reports r
           LEFT JOIN members rep ON rep.id = r.reporter_id
           LEFT JOIN members tgt ON tgt.id = r.reported_member_id
           WHERE r.family_code = ? ORDER BY r.created_at DESC`
        ).bind(privateReportsMatch[1]).all();
        return json({ reports: results });
      }
      // تجاهل بلاغ شات خاص
      const dismissPrivateReportMatch = path.match(/^\/api\/private\/reports\/([A-Z0-9]+)\/([a-zA-Z0-9_]+)\/dismiss$/);
      if (dismissPrivateReportMatch && method === 'POST') {
        const body = await request.json().catch(() => ({}));
        if (!(await isAdminMember(env.DB, dismissPrivateReportMatch[1], body.adminId))) {
          return json({ error: 'مسموح لكبير العائلة بس' }, 403);
        }
        await env.DB.prepare('DELETE FROM private_chat_reports WHERE id = ? AND family_code = ?')
          .bind(dismissPrivateReportMatch[2], dismissPrivateReportMatch[1]).run();
        return json({ ok: true });
      }
