...
<body>
  ...
  <table>
    ...
    <tr><td>الاسم (أو اسم مستعار من اختيارك)</td><td>عند إنشاء عائلة أو الانضمام لها</td><td>تعريف العضو وتسجيل الدخول</td></tr>
  ...
  <div class="box">لا يتم جمع رقم هاتفك أو بريدك الإلكتروني في أي مرحلة. تسجيل الدخول يتم بكود العائلة + اسمك + كلمة السر فقط.<br><br><strong>التطبيق لا يشترط اسمك الحقيقي إطلاقًا</strong> - تقدر تسجّل بأي اسم مستعار تحب تظهر بيه لعائلتك. خلي بالك إن الاسم ده بيبقى وسيلة تسجيل دخولك، فلازم تفتكره بالظبط.</div>
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/api/family/login' && method === 'POST') {
        ...
        if (!trimmedName || !password) return json({ error: 'اكتب اسمك وكلمة السر' }, 400);
        const memberRow = await env.DB.prepare(
          'SELECT * FROM members WHERE family_code = ? AND name = ?'
        ).bind(upperCode, trimmedName).first();
        if (!memberRow) return json({ error: 'كود العائلة أو الاسم غير صحيح' }, 404);
        if (memberRow.status === 'pending') return json({ error: 'pending_approval' }, 403);
        if (!memberRow.pin_hash) {
          // مفيش كلمة سر متسجلة (كبير العائلة عمله reset) - بنعتبر اللي كتبه دلوقتي هو كلمة السر الجديدة
          if (password.length < 6) return json({ error: 'كلمة السر لازم تكون 6 حروف أو أرقام على الأقل' }, 400);
          await env.DB.prepare('UPDATE members SET pin_hash = ? WHERE id = ?').bind(await hashSecret(password), memberRow.id).run();
        } else {
          const valid = await verifySecret(password, memberRow.pin_hash);
          if (!valid) return json({ error: 'كلمة السر غلط' }, 401);
        }
