...
| `rate_limit_migration.sql` | جدول الحد من محاولات الدخول (بديل D1 بدل Cloudflare Rate Limiting) |
| `private_reports_migration.sql` | جدول بلاغات الشات الخاص |
...
مفعّل بالكامل دلوقتي في `wrangler.toml` (`[[durable_objects.bindings]]` + `[[migrations]]`). زر الطوارئ بيبعت إشعار متكرر كل 30 ثانية لباقي العائلة (حتى لو التطبيق مقفول عندهم)، لحد ما حد يضغط "تم الاطلاع" أو لحد أقصى 10 دقايق. لو النشر فشل بسبب الجزء ده، ابعت رسالة الخطأ الكاملة من الترمينال فورًا عشان نشخّصها بدقة بدل التخمين.

### هـ) إعادة تعيين كلمة سر لوحة الإدارة (لو نسيتها)
`ADMIN_KEY` مش مخزّنة في أي ملف - هي secret على مستوى حساب Cloudflare بتاعك. لو نسيتها، سجّل واحدة جديدة تحل محل القديمة تلقائيًا (مفيش داعي تمسح القديمة الأول):
```bash
wrangler secret put ADMIN_KEY
```
هيطلب منك تكتب/تلصق القيمة الجديدة، وخلاص هتشتغل بيها من غير أي تعديل في الكود.
...
- حذف وإبلاغ في الشات الخاص كمان (مش بس العام) - البلاغ بيكشف الرسالة المُبلَّغ عنها بس، بموافقة المُبلِّغ، وباقي المحادثة تفضل مشفرة تمامًا
